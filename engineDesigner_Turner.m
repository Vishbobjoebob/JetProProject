classdef engineDesigner_Turner
    properties
        % % Flight Conditions %%
    ...
    methods
        function obj = engineDesigner_(Ta, pa, M, Pr_c, Pr_f, beta, b, f, f_ib, f_ab)
            obj.Ta = Ta;
            obj.pa = pa;
            obj.M = M;
            obj.Pr_c = Pr_c;
        ...
        function obj = nozzle_core(obj)
            MW = 0.0289;
            ad_eff_n = 0.96;
            cp_n_n = 3.45 + 0.55 * (obj.T06 / 1000)^2 - (0.15 * (obj.T06 / 1000)^3);
            obj.Te = obj.T06 * (1 - ad_eff_n*(1-((obj.pa/obj.p06)^(1/cp_n_n)))); % Normalized cp for core nozzle (cp/R)
            cp_cn = cp_n_n*(8.314/MW);
            obj.ue = sqrt(2*cp_cn*(obj.T06-obj.Te));
            gamma_cn = cp_n_n/(cp_n_n-1);
            obj.Me = obj.ue/sqrt(gamma_cn*(8.314/MW)*obj.Te);
        end
